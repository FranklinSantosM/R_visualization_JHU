...
selectdata <- select(dat, educ, edloan, employ, CC18_308a)
# Problem 2
...
#### Write your code below:
df <- mutate(selectdata,
       trump_approve_disapprove = CC18_308a)
  
df %>% mutate(trump_approve_disapprove = recode(trump_approve_disapprove, 
                                                `1`=1, 
                                                `2`=1, 
                                                `3`=0, 
                                                `4`=0))
...
# Using the full dataset, use summarise() to create a summary table for survey respondents who are employed full time and are married. The table should have the mean, median and standard deviations for the political viewpoint column.


#### Write your code below:
probl <- filter(dat, employ == 1, marstat == 1)
summarise(probl,`Mean Viewpoint`=mean(ideo5), `Median Viewpoint`=median(ideo5))
